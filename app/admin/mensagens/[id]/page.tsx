      ...
      <div className="mx-auto max-w-4xl px-6 py-12">
        ...
        <div className="rounded-2xl border border-mesa-200 bg-white">
          ...
          <ul className="divide-y divide-mesa-100">
            {destinatarios.map((d) => (
              <li key={d.id} className="flex flex-wrap items-center justify-between gap-3 px-6 py-3">
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-mesa-800">
                    {d.aluno_nome || d.aluno_email}
                  </p>
                  {d.aluno_nome && (
                    <p className="truncate text-xs text-mesa-500">{d.aluno_email}</p>
                  )}
                  {d.email_erro && (
                    <p className="mt-1 text-xs text-red-700">{d.email_erro}</p>
                  )}
                </div>
                <div className="flex items-center gap-2 text-xs text-mesa-600">
                  <StatusBadge status={d.email_status} />
                  {d.email_enviado_em && (
                    <span>
                      {new Date(d.email_enviado_em).toLocaleString("pt-BR")}
                    </span>
                  )}
                </div>
              </li>
