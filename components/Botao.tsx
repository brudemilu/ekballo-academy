...
/**
 * Barra de progresso para operação longa sem duração conhecida
 * (gerar áudio, converter vídeo, baixar livro inteiro).
 *
 * Se você SOUBER a porcentagem, passe `valor` — barra determinada
 * é sempre melhor: ela responde "quanto falta", que é a pergunta
 * que a pessoa está fazendo. A indeterminada só diz "ainda estou
 * aqui", e é o último recurso.
 */
export function BarraProgresso({
  ...
  const determinada = typeof valor === "number";
  const pct = determinada ? Math.max(0, Math.min(100, valor)) : undefined;

  return (
    <div className={className}>
      {rotulo && (
        <div className="mb-1.5 flex items-baseline justify-between text-xs text-mesa-600">
          <span>{rotulo}</span>
          {determinada && (
            <span className="tabular-nums font-medium text-mesa-700">
              {Math.round(pct!)}%
            </span>
          )}
        </div>
      ...
      <div
        role="progressbar"
        aria-valuenow={determinada ? Math.round(pct!) : undefined}
        aria-valuemin={0}
        aria-valuemax={100}
        aria-label={rotulo ?? "Progresso"}
        className={[
          "h-1.5 w-full overflow-hidden rounded-full",
          determinada ? "bg-mesa-200" : "progresso-indeterminado",
        ].join(" ")}
      >
        {determinada && (
          <div
