// Aplica a migration 113 (Curso "Oração") via service role.
//
...
const sql = readFileSync(new URL("../supabase/migrations/113_curso_oracao_keller.sql", import.meta.url), "utf8");

const CURSO = {
  slug: "oracao-keller",
  titulo: "Oração",
  descricao: (sql.match(/'oracao-keller',\s*'[^']*?',\s*'([\s\S]*?)',\s*'\/api\/og/) || [])[1],
  imagem_url: "/api/og/curso/oracao-keller",
};
...
if (aulas.length !== 16) throw new Error(`Esperava 16 aulas, achei ${aulas.length}`);
