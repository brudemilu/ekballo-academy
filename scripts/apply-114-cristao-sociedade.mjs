// Aplica a migration 114 (Curso "O Cristão em uma Sociedade não Cristã") via service role.
//
...
const sql = readFileSync(new URL("../supabase/migrations/114_curso_cristao_sociedade.sql", import.meta.url), "utf8");

const CURSO = {
  slug: "cristao-sociedade-nao-crista",
  titulo: "O Cristão em uma Sociedade não Cristã",
  descricao: (sql.match(/'cristao-sociedade-nao-crista',\s*'[^']*?',\s*'([\s\S]*?)',\s*'\/api\/og/) || [])[1],
  imagem_url: "/api/og/curso/cristao-sociedade-nao-crista",
};
...
if (aulas.length !== 17) throw new Error(`Esperava 17 aulas, achei ${aulas.length}`);
