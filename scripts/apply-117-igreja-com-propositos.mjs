// Aplica a migration 117 (Curso "Uma Igreja com Propósitos") via service role.
//
...
const sql = readFileSync(new URL("../supabase/migrations/117_curso_igreja_com_propositos.sql", import.meta.url), "utf8");

const CURSO = {
  slug: "igreja-com-propositos",
  titulo: "Uma Igreja com Propósitos",
  descricao: (sql.match(/'igreja-com-propositos',\s*'[^']*?',\s*'([\s\S]*?)',\s*'\/api\/og/) || [])[1],
  imagem_url: "/api/og/curso/igreja-com-propositos",
};
...
if (aulas.length !== 21) throw new Error(`Esperava 21 aulas, achei ${aulas.length}`);
