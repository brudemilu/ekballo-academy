// Aplica a migration 110 (Curso "Campo de Batalha da Mente") via service role.
//
...
const sql = readFileSync(new URL("../supabase/migrations/110_curso_campo_batalha_mente.sql", import.meta.url), "utf8");

const CURSO = {
  slug: "campo-batalha-mente",
  titulo: "Campo de Batalha da Mente",
  descricao: (sql.match(/'campo-batalha-mente',\s*'Campo de Batalha da Mente',\s*'([\s\S]*?)',\s*'\/api\/og/) || [])[1],
  imagem_url: "/api/og/curso/campo-batalha-mente",
};
...
if (aulas.length !== 28) throw new Error(`Esperava 28 aulas, achei ${aulas.length}`);
