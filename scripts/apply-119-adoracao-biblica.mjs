// Aplica a migration 119 (Curso "Adoração Bíblica" — Russell P. Shedd) via service role.
//
//   node scripts/apply-119-adoracao-biblica.mjs
//
// Lê o .sql como fonte da verdade: faz o parse do curso, das 11 aulas
...
const sql = readFileSync(new URL("../supabase/migrations/119_curso_adoracao_biblica.sql", import.meta.url), "utf8");

const CURSO = {
  slug: "adoracao-biblica",
  titulo: "Adoração Bíblica",
  descricao: (sql.match(/'adoracao-biblica',\s*'[^']*?',\s*'([\s\S]*?)',\s*'\/api\/og/) || [])[1],
  imagem_url: "/api/og/curso/adoracao-biblica",
};
...
if (aulas.length !== 11) throw new Error(`Esperava 11 aulas, achei ${aulas.length}`);
