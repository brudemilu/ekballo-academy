// Aplica a migration 116 (Curso "A Treliça e a Videira") via service role.
//
...
const sql = readFileSync(new URL("../supabase/migrations/116_curso_trelica_videira.sql", import.meta.url), "utf8");

const CURSO = {
  slug: "trelica-videira",
  titulo: "A Treliça e a Videira",
  descricao: (sql.match(/'trelica-videira',\s*'[^']*?',\s*'([\s\S]*?)',\s*'\/api\/og/) || [])[1],
  imagem_url: "/api/og/curso/trelica-videira",
};
...
if (aulas.length !== 13) throw new Error(`Esperava 13 aulas, achei ${aulas.length}`);
