// Aplica a migration 109 (Curso "Contentamento" — Nancy Wilson) via service role.
//
...
const sql = readFileSync(new URL("../supabase/migrations/109_curso_contentamento.sql", import.meta.url), "utf8");

const CURSO = {
  slug: "contentamento",
  titulo: "Contentamento",
  descricao: (sql.match(/'contentamento',\s*'Contentamento',\s*'([\s\S]*?)',\s*'\/api\/og/) || [])[1],
  imagem_url: "/api/og/curso/contentamento",
};
