// Aplica a migration 112 (Curso "Discipulado (Bonhoeffer)") via service role.
//
...
const sql = readFileSync(new URL("../supabase/migrations/112_curso_discipulado_bonhoeffer.sql", import.meta.url), "utf8");

const CURSO = {
  slug: "discipulado-bonhoeffer",
  titulo: "Discipulado (Bonhoeffer)",
  descricao: (sql.match(/'discipulado-bonhoeffer',\s*'[^']*?',\s*'([\s\S]*?)',\s*'\/api\/og/) || [])[1],
  imagem_url: "/api/og/curso/discipulado-bonhoeffer",
};
