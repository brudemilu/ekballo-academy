// Aplica a migration 118 (Curso "JesusCopy") via service role.
//
...
const sql = readFileSync(new URL("../supabase/migrations/118_curso_jesuscopy.sql", import.meta.url), "utf8");

const CURSO = {
  slug: "jesuscopy",
  titulo: "JesusCopy",
  descricao: (sql.match(/'jesuscopy',\s*'[^']*?',\s*'([\s\S]*?)',\s*'\/api\/og/) || [])[1],
  imagem_url: "/api/og/curso/jesuscopy",
};
