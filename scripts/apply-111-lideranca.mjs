// Aplica a migration 111 (Curso "Liderança com Propósitos") via service role.
//
...
const sql = readFileSync(new URL("../supabase/migrations/111_curso_lideranca_aulas.sql", import.meta.url), "utf8");

const CURSO = {
  slug: "lideranca-com-propositos",
  titulo: "Liderança com Propósitos",
  descricao: (sql.match(/'lideranca-com-propositos',\s*'Liderança com Propósitos',\s*'([\s\S]*?)',\s*'\/api\/og/) || [])[1],
...
if (aulas.length !== 12) throw new Error(`Esperava 12 aulas, achei ${aulas.length}`);
