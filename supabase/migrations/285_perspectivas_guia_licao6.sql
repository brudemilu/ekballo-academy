-- 285_perspectivas_guia_licao6.sql
-- Guia de estudo da Lição 6.
--
-- PROCEDÊNCIA: PDF ESCANEADO, sem camada de texto — transcrição de OCR, como
-- os guias das Lições 4 e 5. Garantia menor que a das Lições 1 a 3 e das
-- leituras, onde a extração da camada de texto permitia provar por contagem
-- de caracteres que nenhuma letra faltava.
--
-- O escaneamento sangra a página vizinha na margem, seguindo a paridade do
-- caderno (ímpar pela esquerda, par pela direita) — confirmada de novo aqui,
-- sem exceção nas 8 páginas. A largura do corte foi escolhida por página,
-- testando as opções e medindo quantos títulos de seção sobreviviam a cada
-- uma; um valor fixo ora deixa lixo, ora deceparia os títulos.
--
...
begin
  ...
  if exists (select 1 from aulas a where a.curso_id = v_curso and a.ordem = 29) then
    update aulas set titulo = $t$Lição 6 · Guia de estudo — A expansão protestante e o movimento missionário$t$, conteudo = $conteudo$ESTUDAR ESTA LIÇÃO O AJUDARÁ A:
...
CONCLUSÃO DAS LEITURAS NO NÍVEL AVANÇADO$conteudo$
    ...
    values (v_curso, $t$Lição 6 · Guia de estudo — A expansão protestante e o movimento missionário$t$, $conteudo$ESTUDAR ESTA LIÇÃO O AJUDARÁ A:
...
CONCLUSÃO DAS LEITURAS NO NÍVEL AVANÇADO$conteudo$, 29);
  end if;
end
$curso$;
