-- 275_perspectivas_licao1_rotulos_diagrama.sql
-- O diagrama "O Cumprimento Progressivo da Promessa de Deus" entrou como
-- imagem; os rótulos dele são texto no PDF e passavam a existir só dentro da
-- figura. Agora vão também como texto, logo abaixo dela, na ordem da linha do
-- tempo — nenhuma palavra do original fica fora do texto.
do $curso$
declare
  v_curso uuid;
begin
  select id into v_curso from cursos where slug = 'perspectivas';
  update aulas set conteudo = $conteudo$Nosso Deus tem um propósito global. Ele colocou em nossos corações o desejo de sermos Seus amigos. Temos o desejo de colaborar com Deus nesta tarefa e de vivermos na dignidade de um propósito que é maior do que nossos próprios interesses. Realmente queremos servir a Deus da melhor maneira possível. O que nos impede? Nosso maior problema é que nossa visão é limitada.
