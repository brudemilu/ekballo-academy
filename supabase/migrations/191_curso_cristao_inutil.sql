-- Curso: Como se tornar um cristão inútil (Rodrigo Bibo) — transcrição integral sem perguntas.
do $migration$
declare
  v_curso_id uuid;
  v_aula_id uuid;
...
begin
  ...
  if v_curso_id is null then
    ...
    values (
      ...
      $desc$Estudo guiado a partir de Como se tornar um cristão inútil, de Rodrigo Bibo. Em dez aulas, o livro chama a igreja a redescobrir a alegria de servir sem protagonismo, mérito ou culpa, vivendo como sacerdotes de Cristo na vida comum. Cada aula traz o texto na íntegra, sem perguntas de reflexão.$desc$,
    ...
    set titulo = 'Como se tornar um cristão inútil',
        descricao = $desc$Estudo guiado a partir de Como se tornar um cristão inútil, de Rodrigo Bibo. Em dez aulas, o livro chama a igreja a redescobrir a alegria de servir sem protagonismo, mérito ou culpa, vivendo como sacerdotes de Cristo na vida comum. Cada aula traz o texto na íntegra, sem perguntas de reflexão.$desc$,
...
$conteudo$Preciso começar agradecendo a todos os meus leitores. Vocês levaram O Deus que destrói sonhos muito longe. Vocês me ajudaram a ser lido por milhares de pessoas.
...
E sou grato a Deus, que me deu a rica oportunidade de ter tanta gente legal e amorosa na minha vida, que me usou para escrever uma mensagem simples, mas importante, para sua igreja. É tudo para a tua glória, Deus.$conteudo$)
...
—C M, pastor, professor e teólogo$conteudo$)
    returning id into v_aula_id;
  end if;
...
WRIGHT, N. T. Paulo para todos: 2Coríntios. Rio de Janeiro: Thomas Nelson Brasil, 2020. p. 78.$conteudo$)
    returning id into v_aula_id;
  end if;
  ...
  if v_aula_id is null then
    insert into public.aulas (curso_id, titulo, ordem, conteudo)
    values (v_curso_id, $t$Os novos sacerdotes de Deus$t$, 5,
$conteudo$Certo domingo à noite, eu estava pregando em uma igreja e perguntei: “Quantos de vocês se consideram ministros ou ministras do evangelho?”. Os obreiros atrás de mim levantaram as mãos, e algumas poucas pessoas na plateia também. Então, reformulei: “Quem aqui se considera servo ou serva de Deus?”. O número de mãos que se levantaram foi um pouco maior; contudo, ainda ficou aquela impressão de que a maioria dos cristãos não sabe quem é em Cristo e qual é seu lugar na história que Deus está escrevendo na terra. Esse não foi o único dia em que recebi a mesma resposta a essas perguntas. Infelizmente, muitos irmãos e irmãs, por não terem um título ou um cargo na igreja, não se enxergam como servos/ministros de Deus.
...
MCKNIGHT, Scot; BARRINGER, Laura. Uma igreja chamada TOV: a formação de uma cultura de bondade que resiste a abusos de poder e promove cura. São Paulo: Mundo Cristão, 2022.$conteudo$)
    returning id into v_aula_id;
  end if;
  ...
  if v_aula_id is null then
    insert into public.aulas (curso_id, titulo, ordem, conteudo)
    values (v_curso_id, $t$Enviados para a vida comum$t$, 8,
$conteudo$Quando cheguei à fé, em 1999, minha vida mudou radicalmente. Eu era um jovem típico de 17 anos e tinha as mesmas preocupações e comportamentos que qualquer outro adolescente não cristão. Ao ser aceito como membro de uma comunidade de fé, abracei a vida religiosa com todas as minhas forças. Muito cedo, entendi que minha vida precisava testemunhar sobre Jesus.
...
TYNDALE, W. apud CARRIKER, T. Qual o melhor trabalho para agradar a Deus? Disponível em: https://ultimato.com.br/sites/blogdaultimato/2017/11/24/qual-o-melhor- trabalho-para-agradar-a-deus/. Acesso em: 18 abr. 2024.$conteudo$)
    returning id into v_aula_id;
  end if;
...
KEENER, Craig S. Comentário exegético Atos: introdução e caps 1.1 a 2.47. Rio de Janeiro: CPAD, 2022. p. 830-831.$conteudo$)
    returning id into v_aula_id;
  end if;
end;
$migration$;
