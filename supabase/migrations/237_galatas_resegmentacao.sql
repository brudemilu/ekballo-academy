-- Gálatas para Você — reconstrói a segmentação em capítulos.
--
-- A carga original cortou o livro em 18 mesas, mas o livro tem 13 capítulos.
-- Cinco mesas tinham como título uma frase de destaque tirada do miolo (uma
-- delas com palavra partida pelo OCR) e a mesa 18 virou depósito, com 35 mil
-- caracteres, incluindo as páginas 196-207, posteriores ao fim do sumário.
--
-- Refaz o livro em 14 mesas — o contexto histórico mais os 13 capítulos — pelas
-- faixas de página do sumário (11-195). Cada mesa passa a ter entre 96 e 131
-- parágrafos, contra os cortes arbitrários de antes.
--
-- Autorizado pelo Bruno em 26/ago/2026. Verificado antes: nenhuma das 18 mesas
-- tinha progresso, grifo ou anotação de aluno, então a remoção das mesas 15 a 18
-- não descarta trabalho de ninguém. Aplicar só os UPDATEs sem a remoção faria o
-- texto dos fragmentos aparecer DUPLICADO, já que ele agora vive dentro dos
-- capítulos reconstruídos.
--
-- Ficam de fora ~2.700 palavras das páginas 196-207 (fecho fora do sumário) que
-- a segmentação quebrada havia varrido para a mesa-depósito.
...
begin
  ...
  if v_curso_id is null then
    raise exception 'curso galatas-para-voce nao encontrado';
  end if;

  -- trava de segurança: só remove se de fato não houver trabalho de aluno
  if exists (
    select 1 from public.aulas a
    where a.curso_id = v_curso_id and a.ordem between 15 and 18
      and (exists (select 1 from public.progresso p where p.aula_id = a.id)
        or exists (select 1 from public.destaques_aula g where g.aula_id = a.id)
        or exists (select 1 from public.anotacoes n where n.aula_id = a.id))
  ) then
    raise exception 'ha progresso/grifo/anotacao nas mesas 15-18: abortado';
  end if;

  update public.aulas
  set titulo = $t$O contexto histórico$t$, conteudo = $c$INTRODUÇÃO a G álatas
...
Paulo nos explicará que as verdades do evangelho modificam a vida de alto a baixo; que transformam nosso coração, nosso modo de pensar e nossa abordagem de absolutamente tudo. O evangelho — a mensagem de que somos piores do que jamais ousamos acreditar, porém mais amados e aceitos em Cristo do que jamais ousamos esperar — cria uma nova dinâmica radical para o crescimento pessoal, para a obediência, para o amor.

Gálatas tem tudo que ver com o evangelho, do qual todos necessitamos ao longo da vida inteira. E dinamite pura, e eu oro para que, à medida que ler este livro, a mensagem poderosa da epístola venha a explodir em seu coração e o encha de entusiasmo para ver o mesmo efeito no coração de outras pessoas.

Abaixo, fiz um rápido resumo do cenário histórico da carta; no apêndice, aludi a alguns debates modernos sobre a mensagem que ela contém. Mas, se você quiser passar para o livro de Gálatas em si neste momento, vá para página 13.

T imothy Keller

O contexto histórico

O apóstolo Paulo foi um missionário plantador de igrejas. Depois de abrir uma igreja e deixar a região, continuava a orientar as novas congregações por intermédio de cartas. Uma delas é esta epístola às igrejas cristãs da região da Galácia, na Ásia Menor. A maioria dos estudiosos concorda que Paulo a escreveu por volta de 50 d.C. (de 15 a 20 anos apenas depois da morte de Cristo). É útil identificar os três dados seguintes no cenário histórico, que nos ajudarão a entender a epístola:

INTRODUÇÃO a G álatas

* יA carta trata de uma divisão social e racial nas igrejas da Galácia. Os primeiros cristãos de Jerusalém eram judeus, mas, à medida que o evangelho foi se disseminando a partir desse centro, quantidades crescentes de gentios começaram a receber a Cristo. Contudo, um grupo de mestres gálatas estava agora insistindo em que os cristãos gentios praticassem todos os costumes cerimoniais tradicionais da lei de Moisés, como faziam os cristãos judeus. Ensinavam que os gentios tinham de observar as leis dos alimentos e se circuncidar para ser plenamente aceitos por Deus e ser assim agradáveis a ele. * Embora essa controvérsia específica nos pareça hoje muito remota, Paulo tratou dela como verdade eterna da maior importância. Ele ensinou que as divisões culturais e a desunião nas igrejas da Galácia deviam-se à confusão acerca da natureza do evangelho. Ao insistir em “Cristo e algo mais” como requisito para a plena aceitação por parte de Deus, os mestres estavam apresentando um modo todo diferente de se relacionar com Deus (“outro evangelho”, 1.6), em relação àquele que Paulo lhes transmitira (“um evangelho diferente do que já vos pregamos”, 1.8). Esse evangelho diferente estava criando divisão e conflitos culturais. Paulo lutou contra o “outro evangelho” de maneira enérgica e sem pedir desculpas a ninguém, pois abrir mão do verdadeiro evangelho é abandonar e perder o próprio Cristo (1.6). Portanto, tudo estava em jogo naquele debate. » O fato mais evidente acerca do cenário histórico costuma ser o mais ignorado. Na carta aos gálatas, Paulo expõe em detalhes o que é o evangelho e como ele opera. Mas o público-alvo dessa explanação do evangelho é só de cristãos professos. Isso significa que não são apenas os não cristãos que precisam aprender o evangelho e aplicá-lo em sua vida, mas também os crentes.$c$, audio_leitura_url = null
  where curso_id = v_curso_id and ordem = 1;
  update public.aulas
  set titulo = $t$1. A Singularidade do Evangelho$t$, conteudo = $c$VERSÍCULOS 1-9 GÃLATAS ■ CAPÍTULO 1

1. A SINGULARIDADE DO EVANGELHO

Talvez o que mais chame a atenção na abertura de Gálatas seja o tom usado por Paulo e o estado de espírito por trás dele. Paulo está surpreso. Ele também parece zangado. Sua linguagem, quase desde o início, é extraordinariamente incisiva. Se, como é comum, as cartas de Paulo, após a saudação, dão espaço para uma ação de graças por aqueles para quem ele está escrevendo (veja, por exemplo, Fp 1.3-8, Cl 1.3-8 e 1C0 1.4-9), aqui ele se limita a dizer: “Estou admirado...” (v. 6a). O que despertou sentimentos tão intensos nele?

Desvio

Em primeiro lugar, Paulo fica admirado porque aqueles cristãos recém-convertidos estão se deixando levar por um

evangelho que não é evangelho de fato (v. 7), expondo-se a um enorme perigo. Eles estão perturbados (v. 7b).
...
que vinha ensinando aos cristãos gentios convertidos que eles eram obrigados a cumprir os costumes culturais judaicos da

lei mosaica — em relação ao que comer, à circuncisão e às demais leis cerimoniais — a fim de agradar a Deus de verdade. Para os gálatas, é provável que isso não parecesse uma diferença radical do que lhes fora ensinado. Com certeza, o ponto central da vida cristã é agradar a Deus! Mas Paulo diz: “Isso é uma rejeição completa de tudo que lhes tenho dito”.
...
Não há meio-termo entre a justificação cristã e a justifi

cação pelas obras. Não existe alternativa à justificação cristã,

senão a justificação pelas obras; se você não edificar sua fé so

bre a obra de Cristo, deve edificá-la sobre suas próprias obras.1
...
Primeiro, ensina que as boas obras são suficientes para se chegar a Deus. Se todas as pessoas boas podem conhecer a Deus, então a morte de Jesus não era necessária; basta a

virtude. O problema é que isso quer dizer que pessoas más não têm esperança nenhuma, contradizendo o evangelho, que convida “tanto maus quanto bons”
...
3. Em qual dos três perigos modernos do falso evangelho você ou sua igreja poderíam cair com maior facilidade?$c$, audio_leitura_url = null
  where curso_id = v_curso_id and ordem = 2;
  update public.aulas
  set titulo = $t$2. A Surpreendente Graça de Deus$t$, conteudo = $c$VERSÍCULOS 10-24 GÁLATAS ■ CAPÍTULO 1

2. A SURPREENDENTE GRAÇA DE DEUS

As igrejas costumam pedir a seus membros que compartilhem um testem unho durante o culto ou na reunião de oração. Aqui encontramos o apóstolo Paulo compartilhando o dele. Na verdade, Gálatas 1.10—2.21 costuma ser chamado de seção autobiográfica da epístola, uma vez que Paulo relata sua conversão e experiências cristãs iniciais. Isso não é algo raro para Paulo; nós o vemos falando sobre a própria conversão e experiência em Atos 22.2b-21 e 26.4-23. Aqui, como em Atos, Paulo não está compartilhando seu testemunho para a inspiração dos outros ou para se recomendar a nós. Ele o faz para refutar as afirmações de pessoas que querem enfraquecer sua mensagem, e só quer que seu testemunho aponte para o Deus da surpreendente graça.
...
Isso é espantoso. Paulo resistira a Deus e fizera muita coisa errada (veja At 26.14), mas Deus invalidara todas as suas intenções e usara suas experiências, inclusive os fracassos, para prepará-lo primeiro para a conversão, depois para ser um pregador para os gentios (v. 16). O conhecimento do Antigo Testamento, o zelo, o treinamento, o esforço que ele empregara para se opor a Deus e à sua igreja (v. 13) — tudo estava sendo usado por Deus para quebrantá-lo e capacitá-lo para ser instrumento de Deus na edificação de sua igreja. Deus trabalhara o tempo todo para usar Paulo exatamente no estabelecimento

da fé a que ele se opusera (v. 23).
...
sucesso como escravo (Gn 37.5-8,19,20) — na verdade foi instrumento para estabelecer José como seu libertador (Gn 50.19,20). Os apóstolos repetiam constantemente que as pessoas que se opuseram a Jesus só contribuíram para promover os propósitos divinos (At 2.23; 4.27,28). No final, ficará claro que toda oposição a Deus só serviu para confir

mar e ajudar a concretizar seu desígnio.
...
Imagine um pai que está assistindo seu filho amado jogar no time do qual é treinador. Sentado no banco, ele ama o filho plena e completamente. Se o filho esquece suas instruções e comete um erro, isso em nada mudará seu amor por ele ou sua aprovação. O filho tem o amor do pai assegurado, independen

temente de seu desempenho. A aprovação divina nos liberta para vivermos d a m aneira que Deus aprova.
...
3. Até que ponto você está comprometido em passar tempo com Deus... em passar tempo com outros crentes... em passar tempo contando seu testemunho a outros?$c$, audio_leitura_url = null
  where curso_id = v_curso_id and ordem = 3;
  update public.aulas
  set titulo = $t$3. A Unidade do Evangelho$t$, conteudo = $c$VERSÍCULOS 1-10 GÃLATAS - CAPÍTULO 2

3. A UNIDADE DO EVANGELHO

Deveriamos ler essa passagem com grande temor e gratidão. Ela nos leva a uma reunião em Jerusalém que talvez pareça distante das preocupações dos cristãos do século 21. Mas, na verdade, o que estava em jogo não podia ser mais precioso — foi uma reunião com consequências enormes para todos nós, mesmo hoje. E, como veremos, Deus nos protegeu a todos — inclusive você e eu — naquele dia.
...
Uma coisa era os líderes de Jerusalém aprovarem a conversão

dos gentios; mas conseguiríam aprovar [...] o compromisso

com o Messias sem a inclusão no judaísmo? A visão deles

seria ampla o suficiente para enxergar o evangelho de Cristo

não como um movimento reformista dentro do judaísmo, mas

como boas-novas para o mundo inteiro, e a igreja de Cristo
...
os apóstolos de Jerusalém se mostraram à altura dos acontecimentos e passaram da teoria à prática. Não insistiram na circuncisão de Tito antes de terem comunhão com ele. “Deus

não considera a aparência humana” (v. 6). A aparência exterior tem a ver com nossos atos; a interior, com nosso ser; e o cristianismo tem a ver com quem sou em Cristo, não com o que faço por ele.

Tito era um estudo de caso concreto. Os apóstolos exigiriam

que ele fosse circuncidado ou não?
...
Primeiro, como temos visto, ela significa aceitar qualquer um e todos aqueles que estão “em Cristo Jesus” (v. 4), independentemente de seus antecedentes culturais e étnicos. O cristão norte-americano tem muito mais em comum com o crente no evangelho que leva uma existência nômade nas planícies mon

góis do que com o incrédulo que é seu vizinho de rua, que dirige um carro semelhante ao seu e cujos filhos frequentam a mesma escola que os seus. A unidade cristã não leva em consideração distinções culturais nem depende da similaridade cultural.
...
Por irônico que pareça, se você subadapta ou superadapta, “perde” o evangelho. Se eleva suas tradições à condição de elementos inegociáveis, cria, basicamente, um sistema de

legalismo. E como se dissesse: cristãos verdadeiros sempre
...
A condição dos cristãos judeus [era tal que] sua pobreza pro

vocou a solidariedade das igrejas gentias (veja Rm 15.25-28;
...
são dois grandes anseios do Liberdade e comunhão

coração humano. Nenhum dos dois é satisfeito em definitivo por qualquer visão de mundo ou religião baseada

são dois grandes anseios do coração humano.

em dogmas do tipo “faça por merecer sua salvação”. Eles dividem as pessoas em fileiras culturais e as escravizam emo

cionalmente. É “em Cristo Jesus” que conseguimos desfrutar

da liberdade da aceitação por Deus, independentemente do nosso desempenho, e que podemos desfrutar de uma unidade que não dá atenção nenhuma às fronteiras de países ou culturas. Foi essa unidade e essa liberdade que 0 evangelho de
...
3. Tanto em sua vida pessoal quanto como parte de uma igreja, você tende a superadaptar ou a subadaptar o evangelho à cultura que o cerca?$c$, audio_leitura_url = null
  where curso_id = v_curso_id and ordem = 4;
  update public.aulas
  set titulo = $t$4. Vivendo Conforme o Evangelho$t$, conteudo = $c$VERSÍCULOS Π -21 GÃLATAS ■ CAPÍTULO 2

4. VIVENDO CONFORME O EVANGELHO
...
O que causou essa hipocrisia? O medo (v. 12). É provável que Pedro temesse a crítica daqueles que “eram da circuncisão” — que é como Paulo descre_____________________ _____ ve os mestres da “salvação por j||| Pedro havia

meio de Cristo e mais algu ma coisa”.
...
A abordagem de Paulo faz toda a diferença. Ele não se limitou a dizer: Você está infringindo as regras (embora fosse esse o caso de Pedro), mas: Você se esqueceu do evangelho, esqueceu-se de como foi graciosamente recebido em Cristo. Paulo não se

concentrou tanto no comportamento pecaminoso, mas, sim, na atitude pecaminosa hipócrita por trás dele.

Paulo nâo se concentrou tanto no comportamento pecaminoso, mas, sim, na atitude pecam inosa hipócrita por trás dele.

Esse é o jeito cristão de fazer “oposição” a alguém. Quando você está tentando motivar pessoas estimulando-as a ver suas riquezas e
...
Se até um apóstolo como Pedro precisou aprender mais sobre o que significa ser justificado pela fé, é provável que conosco aconteça a m esm a coisa! fgg

como vemos aqui, se até um apóstolo como Pedro precisou aprender mais sobre o que significa ser justificado pela fé, é provável que conosco aconteça a mesma coisa!
...
“Justificar”, na Bíblia, significa [...] declarar [...], acerca de um

homem levado a julgamento, que ele não está sujeito a nenhuma

penalidade, mas tem direito a todos os privilégios devidos a

quem guarda a lei. Justificar é o ato do juiz que pronuncia sen

tença oposta à condenação — de indulto e imunidade legal.1
...
Porque morreu para a lei, Paulo agora pode “viver para Deus”. A dedução aqui é que, antes de se render à fé, quando ainda tentava se salvar pela observância da lei, Paulo nunca viveu de verdade

para Deus. Fora muito correto e bom — mas tudo por si próprio, nunca para Deus.
...
Veremos muito mais a esse respeito em Gálatas 5. Por ora, Paulo quer nos fazer entender que nossa aceitação nos dá um motivo novo e mais forte para obedecer a Deus; um mo

tivo mais forte do que a justificação pelas obras jamais conseguiría produzir.
...
3. Como você expÜcaria a diferença entre ser moralmente correto e ser cristão para alguém que pensa que ser bom o torna aceitável para Deus?$c$, audio_leitura_url = null
  where curso_id = v_curso_id and ordem = 5;
  update public.aulas
  set titulo = $t$5. Você Nunca Deixa o Evangelho para Trás$t$, conteudo = $c$VERSÍCULOS 1-14 GÃLATAS ■ CAPÍTULO 3

5. VOCÊ NUNCA DEIXA O EVANGELHO PARA TRÁS

Os cinco primeiros versículos do capítulo 3 fazem uma afirmação extraordinária — muito negligenciada pelos cristãos, mas absolutamente crítica para eles.
...
Nos versículos 1-3, Paulo lembra aos cristãos da Galácia como foi que eles se entregaram a Cristo, provenientes do

paganismo. Em essência, Jesus Cristo é retratado “como crucificado” (v. 1). Esse retrato é alcançado pela pregação, por meio do “que ouvistes” (v. 2,5). Paulo não fala de uma imagem literal, mas de uma metáfora.
...
Os cristãos pensam que somos salvos pelo evangelho, mas

que depois crescemos aplicando princípios bíblicos a todas
...
que sejam. Isso deixa você supersensível à crítica, intimidado e com inveja de quem o ofusca. Você fica nervoso e receoso (por

não saber bem em que posição se encontra), ou então vaidoso e se gabando (por tentar se convencer da própria posição). De um jeito ou de outro, você vive com a sensação de maldição e condenação.
...
3. Qual verdade em Gálatas 3.13,14 mais o entusiasma hoje?$c$, audio_leitura_url = null
  where curso_id = v_curso_id and ordem = 6;
  update public.aulas
  set titulo = $t$6. A Lei na Vida do Evangelho$t$, conteudo = $c$VERSÍCULOS 15-25 GÃLATAS ■ CAPÍTULO 3

6. A LEI NA VIDA DO EVANGELHO

Sempre que ouvimos as afirmações radicais da salvação pela graça, deveriamos, na mesma hora, nos sentir compelidos a perguntar: Se estamos “livres da lei”, quer dizer que não temos de obedecer à lei de Deus? Se sou sempre salvo só pela atuação de Cristo e não pela minha, por que deveria lutar para levar uma vida santa? Tenho alguma obrigação de guardar a lei de Deus? Por quê?
...
A promessa de Deus a Abraão é uma promessa de afiança. Trata-se de uma afiança que em nada depende de Abraão, só de Deus. Ele preferiria morrer a quebrar a promessa de bênção a Abraão e a seus descendentes — e morreu de fato, por meio de um descendente em particular (“teu descendente”, G13.16),

de modo a oferecer a bênção ao mundo. Esse “descendente”, o homem Jesus Cristo,
...
Paulo apenas chama a atenção dos gálatas para a impossibilidade de Deus acrescentar exigências de obediência à sua promessa feita em afiança. Ele próprio garantira que a cumpriría. Então, como e por que motivo havería a lei de anular “o testa
...
Assim, o Antigo Testamento requer que as pessoas amem “o Senhor, [seu] Deus, de todo o [seu] coração” (Dt 6.5) e que

sejamos o povo “em cujo coração está a minha [de Deus] lei” (Is 51.7). A lei (se de fato lhe dermos ouvidos) enfatiza sem parar que necessitamos de justiça, de poder, de um amor por Deus que estão além de nós mesmos e além da lei. Necessitamos de salvação pela graça.
...
-las. A lei expôs o pecado, provocou-o, condenou-o. O pro

pósito da lei era erguer a capa de respeitabilidade do homem

e revelar como ele de fato era por baixo — pecador, rebelde,

culpado, sob julgamento de Deus e impotente para se salvar.

Ainda hoje a lei deve ter permissão para cumprir a tarefa

dada por Deus. Um dos grandes erros da igreja contempo

rânea é a tendência de amenizar o pecado e o juízo. [...]

Nunca devemos contornar a lei e ir diretamente ao evan

gelho. Fazer isso é contradizer o plano de Deus na história

bíblica. [...] Nenhum homem jamais soube apreciar o evan

gelho até que a lei lhe fosse revelada primeiro. Só contra

a escuridão do céu noturno, densa como tinta, as estrelas
...
Portanto, Paulo não está indicando que não mais temos qualquer relação com os valores da lei de Deus, mas que não a vemos mais como um sistema de salvação. Ela não impõe mais a obediência por meio da coerção e do medo. O evangelho significa que não mais obedecemos à lei por medo de rejeição e esperança de salvação pelo desempenho. Mas, quando nos apossamos da salvação pela promessa, nosso coração se enche

de gratidão e do desejo de ser como nosso Salvador e de agradar a ele — e a maneira de fazer isso é obedecendo à lei. Uma vez que recorremos à lei motivados pela gratidão, somos melhores em nossa obediência à lei do que jamais fomos antes, quando acreditávamos que essa obediência podería nos salvar.
...
A lei nos mostra como de fato somos. Assim, ela nos chama para vermos Cristo como ele de fato é: nosso Salvador, aquele que cumpriu a lei em nosso benefício e depois morreu

em nosso lugar para que pudéssemos receber a bênção prometida. A lei nos permite amar Jesus e nos capacita a mostrar nosso amor em grata obediência a ele.
...
3. Por que você obedece à lei de Deus? Já aconteceu de obedecer a ela pelos motivos errados?$c$, audio_leitura_url = null
  where curso_id = v_curso_id and ordem = 7;
  update public.aulas
  set titulo = $t$7. Filhos de Deus$t$, conteudo = $c$GÁLATAS ■ CAPÍTULO 3
...
A ideia de que somos filhos de Deus, filhos e filhas do pró

prio Deus [...] é a mola propulsora da vida cristã. [...] Nossa

condição de filhos de Deus é o ápice da criação e o objetivo

da redenção.1

Se quisermos entender quem é o cristão e por que ser cristão é um privilégio, temos de saber dar valor à adoção divina. Se Jesus, na condição de “o descendente” (G1 3.19), tem todas as bênçãos prometidas

a,. _. m Se quisermos entender a Abraao, então, automati|g| M camente, qualquer um que Ej q u e m é O cristão e _ pertença a Cristo pela fé se |f | P or c?u e ser cristáo ® torna herdeiro da promessa | | | u m P rivilégio, temos de Abraão (v. 29). Como essa | | | saber dar valor à herança chega até nós? Por ■ü adoçáo divina. meio do Filho, tornamo-nos filhos de Deus por direito legal (4.4,5), recebendo um novo status; e, por meio do Espírito, tornamo-nos filhos de Deus por experiência (v. 6,7).
...
(veja Gn 3.7,21). Dizer que Cristo é nossa roupa é afirmar que, aos olhos de Deus, somos amados devido à obra e à salvação de Jesus. Quando Deus olha para nós, enxerga-nos como seus filhos porque vê seu Filho. O senhor Jesus nos

tem dado sua justiça e perfeição para usar.
...
Para ilustrar nossa condição de filhos, ele usa a imagem da criança que herda uma grande propriedade. Enquanto menor

de idade, ela “em nada é diferente de um escravo” (4.1), uma vez que “está [sujeita] a tutores e administradores” (v. 2). Quando alcança a maturidade, no entanto, toma posse de sua herança.
...
3. Quando você corre mais perigo de viver como um escravo e não como filho?$c$, audio_leitura_url = null
  where curso_id = v_curso_id and ordem = 8;
  update public.aulas
  set titulo = $t$8. Duas Religiões, Dois Ministérios$t$, conteudo = $c$VERSÍCULOS 8-20 GÁLATAS ■ CAPÍTULO 4

8. DUAS RELIGIÕES, DOIS MINISTÉRIOS
...
O que a expressão significa? Com frequência essas palavras em grego antigo se referiam aos elementos do mundo material

e visível que compõem a natureza: fogo, água, ar e terra. Também costumavam aludir à crença pagã de que forças
...
nós em um salvador, portanto, um “deus”. A justiça pelas obras sempre cria ídolos; acontece, porém, que os falsos salvadores por ela zidos — a frequência à igreja, o ministério em benefício do próximo, a leitura da Bíblia — são coisas que, via de regra, não pensaríamos como ídolos.
...
O exemplo perfeito disso é o relato de Jesus em Lucas 15 sobre dois irmãos. Certo pai tinha um filho imoral, mais novo,

pródigo, e um filho mais velho muito correto. Ambos desejavam não o pai, mas o controle da sua riqueza. Ambos viviam

ind iferen tes ao coração do pai. No fim da história, contudo, o filho imoral se arrepende e vai ao encontro do pai, enquanto o correto permanece do lado de fora, com raiva.
...
Cristãos que não têm mais certeza de serem amados e aceitos

por Deus em Jesus, independentemente de suas realizações

espirituais presentes, são pessoas de uma insegurança

subconsciente radical, mais do que os não cristãos, devido

aos informes constantes que recebem de seu ambiente cristão

acerca da santidade de Deus e da justiça que se espera deles

próprios. Sua insegurança se traduz em orgulho e em duas

atitudes defensivas: a feroz afirmação da própria retidão e a

censura dos outros. Agarram-se desesperados à justiça legal,

farisaica, mas de sua insegurança básica brotam a inveja, o

ciúme e outros [...] pecados.1
...
conhecim ento que ele tem de nós em Cristo. A flitos, tentam os

consolidar uma autoim agem positiva com o uso de nossos ido

los. Paulo lembra que o evangelho mostra que não precisamos

nos fazer belos ou dignos de sermos amados para Deus; ele já

nos conhece. Se é assim, não precisamos criar um ídolo a partir

da aprovação alheia ou m esm o de nossa própria aprovação.
...
Mais presente para a aguçada visão da fé do que parecem

meus objetos exteriores.

Mais querido, mais intimamente próximo até do que o mais
...
Isso quer dizer que precisam, no nível das emoções, ter quem precise deles emocionalmente. Necessitam que seus convertidos e discípulos vivam envolvidos por todos os lados com seus lideres, obedecendo-lhes e adorando-os. Só isso lhes pode assegurar

que são crentes bons e grandiosos, abençoados de fato e favorecidos por Deus.

Os íalsos mestres precisam ter quem precise deles.

Esse objetivo afeta os recursos que usam. Querem “que

vocês também mostrem zelo por eles” (v. 17). Isso é o mesmo que dizer: Eles falam 0 que vocês querem ouvir; fazem cócegas em seus ouvidos, patrocinando a satisfação dos desejos escusos que vocês
...
3. Todos podemos ser ministros do evangelho a outros cristãos. Como seu ministério mudará, baseado nesta seção da Bíblia?$c$, audio_leitura_url = null
  where curso_id = v_curso_id and ordem = 9;
  update public.aulas
  set titulo = $t$9. Graça para com a Estéril$t$, conteudo = $c$VERSÍCULOS 21-31 GÁLATAS ■ CAPÍTULO 4

9. GRAÇA PARA COM A ESTÉRIL

Esses versículos são explosivos. Concluem os temas que Paulo vem destacando desde a metade do capítulo 2. A ideia que ele defende aqui é não apenas que o evangelho converte qualquer um em filho de Deus, mas que os mais orgulhosos, corretos e religiosamente “aptos” costumam ser os que ficam de fora da família de Deus. O evangelho subverte os valores do mundo.
...
Paulo escreve, mas pessoas cuja religião é legalista, que ima

ginam que o caminho para Deus passa pela observância de

determinadas regras.1
...
Dormindo com Agar, Abraão escolhera se fiar em suas próprias capacidades. Optara por “trabalhar” e ganhar seu filho. Agira por fé, mas a fé que depositava em si mesmo, como seu
...
a graça, mas com “a Jerusalém do alto [que] é livre”. No sen

tido em que simboliza aqueles que aprenderam a abrir mão das tentativas de alcançar a salvação e a deixar que Deus os salve, ela “é a nossa mãe” (v. 26). A “cidade-mãe” era o lugar onde o indivíduo se sentia em casa, um cidadão, com direitos.
...
O pastor de uma igreja no Harlem, cujos membros, em sua maioria, são negros, uma vez me contou que sua congregação

foi fundada mais de 80 anos

atrás por uma senhora alemã moradora de Manhattan. Ela era uma cristã dedicada e, por meio do seu estudo bíblico, duas mulheres afro-america

Quem tem mais descobrirá a ruína de suas íalsas estratégias de valor próprio.

nas do Harlem se entregaram a Cristo. Elas então lhe pediram para começar um ministério no Harlem com o intuito de alcan
...
A perseguição da verdadeira igreja [...] nem sempre é patro

cinada pelo mundo, como são chamados os estranhos [...],

mas por nossos meios-irmãos, por pessoas religiosas, pela

igreja nominal. Os maiores inimigos da fé evangélica hoje

não são os descrentes [...], mas a igreja, o sistema, a hierar

quia. Isaque é sempre ohjcto de zombaria e perseguição por
...
2Op. cit., p. 127.$c$, audio_leitura_url = null
  where curso_id = v_curso_id and ordem = 10;
  update public.aulas
  set titulo = $t$10. A Liberdade do Evangelho$t$, conteudo = $c$VERSÍCULOS 1-15 GÃLATAS ■ CAPÍTULO 5

10. A LIBERDADE DO EVANGELHO
...
Pelo que aguardamos? Justiça quer dizer mais do que bondade; é ter uma ficha toda limpa com Deus e manter com ele um relacionamento da mais absoluta retidão. Paulo está afirmando que podemos viver hoje à luz da nossa acolhida e

glorificação futura, certa e garantida nos braços de Deus, pois sabemos que, “se és filho, és também herdeiro por obra de Deus” (4.7). Nenhum outro ser humano, ninguém do mundo

secular, nenhum seguidor de qualquer outra religião pode encarar o próprio futuro dessa maneira! Pessoas não religiosas não fazem ideia de onde estarão dentro de um milhão de anos, e pessoas religiosas sem o evangelho vivem ansiosas acerca de onde estarão, sem conseguir relaxar ou olhar para a frente com entusiasmo. A certeza do nosso futuro com Deus é fruto do evangelho.
...
Mas agora que estamos salvos por completo e somos livres pela graça, estamos mais obrigados do que nunca a obedecer a lei! Por quê? Porque temos mais motivos para amar a Deus do que antes. O amor brota da fé e da esperança do evangelho (v. 5,6) e transborda em amor e serviço para nosso próximo, em vez de usá-los para nos servir. E amar nosso próximo é “toda a lei [que] se resume numa só ordenança” (v. 14).

Temos mais motivos para am ar a Deus do que antes. I

Ou seja, os cristãos estão livres da lei como modo de conquistar mérito diante de Deus, mas não como forma de agradar a Deus. Pelo contrário, a obrigação cresceu. Pois a lei é uma expressão da natureza e do coração de Deus, e, assim, agora somos devedores em dobro perante ele de usá-la para agradá-lo e imitá-lo. Devemos isso a ele como nosso Criador, uma vez que ele nos projetou, é nosso dono e tem tanto o conhecimento para saber como devemos viver quanto o direito de exigir que vivamos dessa maneira. Por outro lado, agora também lhe somos devedores como nosso Redentor, uma vez que queremos agradar, por gratidão, àquele que nos salvou a um custo tão imensurável.
...
3. Pense em um pecado contra o qual você vem lutando. Por que você deseja pecar dessa maneira? Como sua liberdade no evangelho corrói essa motivação para o pecado?$c$, audio_leitura_url = null
  where curso_id = v_curso_id and ordem = 11;
  update public.aulas
  set titulo = $t$11. O Caráter do Evangelho$t$, conteudo = $c$VERSÍCULOS 16-25 GÃLATAS ■ CAPÍTULO 5

11. O CARÁTER DO EVANGELHO

Na religião normal, a motivação para uma vida moral se baseia no medo. No cristianismo fundamentado no evangelho, a motivação é uma dinâmica de amor, como vimos no último capítulo (5.6,14). Agora, Paulo explicita como crescemos em caráter por meio dessa nova dinâmica. E sua manchete é: Crescemos à medida que lutamos.
...
sobrenaturalmente quando nos tornamos cristãos e começou a renovação que consiste agora em nossa “nova natureza”. Por isso, em Efésios 4.22-24, Paulo se refere a essa oposição sarx versus o Espírito como a competição entre o “velho homcm” e o “novo homem”.
...
Testamento para nossa tendência a nos afastarmos de Deus,

então “desejos” (epithumia) é o termo característico e resumi

do do Novo Testamento para o mesmo movimento [...]. O

Novo Testamento funde o conceito de idolatria e de desejos

excessivos que governam a vida [...] para falar de concupis

cência, cobiça, anseio e exigência gananciosa (E f 5.5; Cl 3.5).1
...
O Espírito, então, anseia por nos mostrar a Cristo e nos

conformar com Cristo. Em última análise, isso é o que os cristãos também querem. E fácil não perceber, mas Paulo faz uma declaração extremamente reveladora quando diz que o Espírito e a sarx “se opõem um ao outro, de modo que não conseguis fazer o que quereis” (G15.17). A passagem é paralela a Romanos 7.22,23, quando ele diz que “no que diz respeito ao homem interior, tenho prazer na lei de Deus”, contudo, desçobre que existe “nos membros do meu corpo outra lei guerreando contra a lei da minha mente”.

1“Idols of the heart and,vanity fair’”, The Journal o f Biblical Counseling, vol. 13, n. 2, Winter 1995, p. 36.

G álatas 5.16-25

Viver do jeito do Espírito é o que mais desejamos; todavia, nossa natureza pecaminosa continua a gerar desejos alternativos e concorrentes, os quais experimentamos e aos quais podemos ceder, mas que agora contradizem nosso amor e objetivos mais

eternos. A pessoa nascida de

novo tem tanto desejos pecaminosos quanto divinos, mas devemos querer de verdade o que nosso coração renovado pelo Espírito deseja.

O Espírito anseia por nos conformar com Cristo. Em última análise, isso é o que os cristãos tam bém querem.

Essa declaração está carregada de esperança e afir

mação. Mesmo quando resvalamos para o pecado, podemos repetir com Paulo: Esse não é meu verdadeiro eu; isso não é 0 que eu desejo de verdade. Eu quero Deus e sua vontade.
...
A seguir, nos versículos 20 e 21, surgem oito palavras que descrevem como a carne destrói os relacionamentos. Quatro delas são atitudes destrutivas: “ambição egoísta” (eritheia), ou seja, a competitividade, uma motivação voltada para a busca do eu; “inveja” (phthonoi), a cobiça, o desejo do que os outros têm; “ciúmes” {zelos), o zelo e a energia que vêm de um ego faminto; e “rivalidade” (echthrai), designando a hostilidade, uma atitude antagônica. Quatro descrevem os resultados dessas atitudes nos relacionamentos: “inimizades” (eris), ser polêmico ou gostar de provocar brigas; “ira” (thumoi), explosões de raiva; “discórdias” (dichostaiai), divisões entre pessoas (ao que a

ira conduz); e “partidarismo” {aireseis), grupos partidários e rivais permanentes.

A carne destrói os relacionamentos. I

Por fim, há duas palavras que se referem ao abuso de substâncias: “bebedeiras” e “orgias”. Essas duas palavras estão conectadas. As orgias não são sexuais, mas de bebedices. Uma das obras da carne é o vício em substâncias e comportamentos que criam o prazer.
...
1. Agape = amor. Significa servir uma pessoa para o bem dela

e por seu valor intrínseco, não pelo que ela pode nos proporcionar. Seu oposto é o medo: autoproteção e abusar das
...
3. Quais são os ídolos que precisam ser identificados e desmantelados em sua vida? Como você pode substituí-los por Cristo?$c$, audio_leitura_url = null
  where curso_id = v_curso_id and ordem = 12;
  update public.aulas
  set titulo = $t$12. Relacionamentos do Evangelho$t$, conteudo = $c$VERSÍCULO 26
...
12. RELACIONAMENTOS DO EVANGELHO

Que diferença o evangelho faz em seus relacionamentos? De que maneira ele afeta o modo como você se vê em relação àqueles que o cercam? E como você vê os outros em relação a si mesmo?
...
profundo. Tanto a pessoa que se sente superior quanto a que se sente inferior estão absortas em si mesmas. Em ambos os casos, você concentra todas as suas forças em como o outro faz com que você se sinta ou pareça, em vez de em como você faz com que

o outro se pareça ou sinta. Outra maneira de explicar

Tanto a pessoa que se sente superior quanto a que se sente inferior

isso é em termos de justiça pelas obras, o que estabelece uma ponte entre o versículo 26 e o tema subjacente da carta: a necessidade de viver

estão absortas em si mesmas.
...
Mas o evangelho cria uma nova autoimagem, como vimos antes. Ele me humilha diante de todo o mundo, dizendo-me

que sou um pecador salvo só pela graça. Mas também me encoraja diante de todo o mundo, dizendo-me que sou amado e honrado pelos únicos olhos do universo que de
...
Vejamos outro exemplo, nos relacionamentos sexuais. O moralista tende a ver o sexo como algo sujo, ou pelo menos como um impulso perigoso,

que o tempo todo leva a pecar. A consciência apreensiva do moralista o levará ou a evitar por completo a experiência sexual ou a desenvolver uma necessidade mal direcionada e impetuosa de vivenciá-la. Ambas as possibilidades provêm de um vácuo de glória interior, capaz de fazer do sexo um modo de preencher o vazio.
...
Qual será seu objetivo? “Restaurar essa pessoa com espírito de humildade” (v. 1). O termo grego traduzido como “restaurar” é katartizdo. Esse era o termo usado para pôr no lugar um osso deslocado. Um osso que se desloca provoca dor extrema porque não está mantendo o relacionamento natural, para o qual foi projetado, com as demais partes do corpo. É inevitável que o ato de pôr um osso no lugar produza dor, mas é a dor da cura. Significa que devemos confrontar, mesmo quando for doloroso, mas nosso confronto deve ter como objetivo instigar

a mudança de vida e de coração.

Todavia, o “irmão” confrontará “com espírito de humildade”.

Paulo diz que essa gentileza só lhe sobrevirá se você cuidar de si

mesmo, para não ser tentado também (v. 1). Um conselho difícil, mas prático. Não conseguiremos ser encantadores ao confrontar
...
“os fardos uns dos outros” (v. 2) quando “cada um carregará o

seu próprio fardo” (v. 5)? A resposta está no fato de que existem dois tipos de fardo aqui. O termo grego traduzido como “fardos” no versículo 2 é um grande peso, mas, no versículo 5, o termo também traduzido como “fardo” é diferente e se refere a uma espccic de mochila. O versículo 5 fala que Deus tem dado a cada um de nós um conjunto diferente dc dificuldades e oportunidades, dc fraquezas e de dons. Esse é o “fardo” que nos pertence — nossa responsabilidade diante de Deus.

Portanto, não devemos nos comparar com os outros. Em vez disso, temos de olhar para nossas provações e deveres particulares e reagir a eles em obediência. Se encararmos a vida dessa maneira, julgaremos nosso dia a dia tendo como padrão quem temos sido e quem poderiamos ter sido. Quando virmos progresso, tiraremos dele o orgulho legítimo, sejamos ou não melhores ou piores do que outra pessoa qualquer. Não nos compararemos com alguém que fez menos do que nós (para experimentar o orgulho pretensioso) ou com alguém que fez mais (para sentir desespero ou inveja pretensiosos). Deus nos

tem dado um fardo diferente para carregar e com o qual podemos servi-lo. Nossa tarefa é carregar o nosso fardo individual, não o de mais ninguém, de modo a agradar a Deus.

Nossa tarefa é

carregar o nosso fardo individual de modo a agradar a Deus.

Se encararmos a vida dessa maneira, seremos tardios em julgar os outros também. Seremos acríticos e generosos. Por exemplo, se virmos alguém impaciente, pensaremos: Não sei que pressões essa pessoa está enfrentando, nem com que nível de autocontrole emocional ela começou. Na verdade, talvez, esteja sen

do mais obediente a Deus do que eu hoje!

G álatas 5.26—6.5

Com humildade e gentileza, devemos ajudar os outros em suas tarefas e problemas, com todos os seus fardos. Mas:

Há um fardo que não podemos compartilhar [...] e que é nossa responsabilidade para com Deus no dia do julgamento. Naquele dia, você não poderá carregar minha mochila, nem eu a sua.1
...
1John R. W. Stott, The message o f Galatians. The Bible Speaks Today (Downers Grove: IVP, 1968), p. 160. [edição em português: A mensagem de Gálatas (São Paulo: ABU, 1997)J.$c$, audio_leitura_url = null
  where curso_id = v_curso_id and ordem = 13;
  update public.aulas
  set titulo = $t$13. Semeando e Colhendo$t$, conteudo = $c$VERSÍCULOS 6-18 GÃLATAS ■ CAPÍTULO 6

13. SEMEANDO E COLHENDO

As palavras finais de Paulo podem, à primeira vista, parecer uma série de declarações desconexas, mas na verdade ele está fazendo duas coisas ao mesmo tempo em que anuncia sua despedida. Os versículos 6-10 são uma advertência final, e os versículos 11-18, um convite final. A advertência e o convite de Paulo são basicamente a mesma mensagem, aquela que dá sustentação a cada linha da carta: vivam pelo evangelho!
...
Todo cristão deve se enquadrar na descrição daquele “que está sendo instruído na palavra” (v. 6). O termo grego para “o que está sendo instruído” é katechoumeno$\ aquele que é

catequizado. Isso mostra como era importante oferecer aos
...
Portanto, deveriamos dar com generosidade às equipes de nossas igrejas. Não como “consumidores” que vão a uma igreja

e se limitam a saqueá-la de seus benefícios, sem contribuir de maneira significativa com ela. Mas essa contribuição precisa se fazer acompanhar da atitude correta. O ensino cristão não é só mais
...
A ruína que colhemos vem da ruptura do “tecido” do universo moral, da mesma forma como determinado comporta

mento tem o poder de romper o tecido e a coesão físicos. Existem inúmeras maneiras pelas quais a semeadura para satisfazer a carne colhe ruína. O livro inteiro de Provérbios é resumido por Gálatas 6.7,8! Semear desonestidade rompe o tecido dos relacionamentos e cria a ruína da solidão. Semear inveja e ciúmes rompe o tecido da satisfação e cria a ruína da amargura. E assim por diante, para sempre.

I O pecado faz as coisas se desintegrarem.
...
Mas Paulo também advertiu que, se o evangelho é rejeitado e é adotada formal e completamente a justiça pelas obras, a escravidão e a destruição também serão completas. E provável que ele tivesse em vista ambos os níveis aqui. Se, como cristãos, falhamos em aplicar o evangelho e vivemos “na carne”, tentando fazer por merecer nossa salvação por outros meios, deparamos com uma perda da coerência, da alegria e da força na nossa vida. E se alguém rejeita o evangelho e vive só para a carne, buscando e servindo alguma outra coisa que não Cristo como seu salvador, então esse alguém colherá ruína eterna, em vez de vida eterna.

A advertência é dura, mas a promessa é maravilhosa. I

A advertência é dura, mas a promessa é maravilhosa. “Quem semeia para o Espírito, do Espírito colherá a vida eterna” (v. 8). Se vivermos pelo Espírito, desfrutaremos da aprovação, da certeza, da satisfação e da alegria da vida cristã agora e saberemos que ela continuará além da morte.
...
ao nosso alcance minimizar, seja material, social ou espiritual. Essa pequena frase mostra que o ministério cristão engloba ajudar em uma casa de recuperação tanto quanto explicar para

alguém como entregar sua vida a Cristo.

Esse amor deve ser direcionado “a todos” (v. 10). Para não nos sentirmos sufocados de imediato, Paulo já havia acrescentado “enquanto temos oportunidade” — não se espera que satisfaçamos pessoalmente todas as necessidades de todo

o mundo! Devemos olhar à nossa volta e ver onde esta
...
to, esse amor é para ser dado “aos da família da fé” (v. 10), uma expressão maravilhosa

discipulado.

que mostra que todos os cristãos são uma família. Os cristãos são todos irmãos e irmãs na casa de Deus (4.5). Temos de fazer o bem exaustivamente àqueles que participam da mesma
...
Portanto, ele confirma o que disse lá atrás, em 5.6: “Nem a circuncisão nem a incircuncisão são coisa alguma, mas, sim, o ser nova criação” (6.15). As realizações religiosas ou morais e os fracassos religiosos ou morais são irrelevantes quando

se trata de salvação, porque ela não tem nada a ver com o que eu fiz, mas, sim, com o que Cristo fez.
...
E então ele se despede. Mas, até nesse momento, Paulo lembra os gálatas da mensagem de sua carta. “A graça do nosso Senhor Jesus Cristo” (v. 18) é a porta de entrada para a vida
...
3. Se tivesse de resumir a mensagem de todo o livro de Gálatas em poucas palavras, o que você diria?$c$, audio_leitura_url = null
  where curso_id = v_curso_id and ordem = 14;

  delete from public.aulas
  where curso_id = v_curso_id and ordem between 15 and 18;
  get diagnostics v_removidas = row_count;
  raise notice 'mesas removidas: %', v_removidas;

  update public.cursos set audio_status = 'nenhum', audio_progresso = 0
  where id = v_curso_id;
end
$migration$;
